...
- **Invoice popups (`NewMessageNotification`)**:
  ...
  - **`PayBondInvoice`** (Mostro **Phase 1.5+** taker bond / **Phase 5+** maker bond) maps to a dedicated bond popup mode (`render_pay_bond_invoice` in `src/ui/message_notification.rs`). It mirrors the PayInvoice layout but uses a **🛡️** title (`Anti-abuse Bond Invoice`), a taker or maker amount label (via `MessageNotification.maker_bond_publish`: "Bond invoice to pay" vs "Pay bond to publish your order"), and a yellow "Locked, not spent — refunded on normal completion" disclaimer. If the bond BOLT11 has **expired**, the disclaimer turns red ("⚠️ Bond invoice expired — cancel the order to retake", shortened on narrow terminals) and the popup renders the copyable text card instead of a payable-looking QR. The bond BOLT11 is persisted in `orders.bond_invoice` (`Order::update_bond_invoice`) and re-injected into the Messages row on startup for bond-pending statuses — including a taker row still at `pending` with a stored bond invoice — so **Enter** on the Messages row reopens the bond QR after **Esc** or an app restart; the STATUS banner advertises the reopen shortcut only while the row carries an unexpired invoice (an expired one prompts to cancel instead). Primary button is **Acknowledge** (closes the popup; payment happens in the user's wallet); **Cancel Order** is still wired to `Action::Cancel`. The popup is gated on `order_status ∈ {WaitingTakerBond, WaitingMakerBond, Pending, None}` and role (`local_user_must_act_on_invoice_popup` — status-based for `PayBondInvoice`, not listing kind). `Pending` is included because the daemon deliberately embeds the public-book bucket (not a bond status) in the `PayBondInvoice` payload — the order stays pending on the wire until the bond locks — so live bond rows sit at `Pending` while unpaid; such rows show **Bond Invoice** (not "Pending order") in the sidebar and "Bond payment required" in the STATUS banner. Sync paths: **`take_order`** (taker) and **`send_new_order`** (maker) return `PaymentRequestRequired`. Bonds are **configurable in mostrod** — when not enabled, create/take flows skip this popup.
  - All three popups provide two actions (`Primary` + `Cancel Order`) via Left/Right selection; Enter confirms the selected action.
  - `PayInvoice` and `PayBondInvoice` show a half-block QR when it fits (`src/ui/helpers/qr.rs`). **SPACE** toggles QR/text, **C** copies the raw bolt11, **X** sends **Cancel Order**, and **Up/Down** / **PageUp/PageDown** scroll the text view. QR mode folds the order id into the title and draws a 1-row **Acknowledge** / **Cancel Order** strip (Left/Right still move the highlight; Enter confirms). If the terminal cannot fit the QR, the popup falls back to the wrapped invoice immediately and keeps the same Ack/Cancel strip on short heights. An **expired bond invoice** never renders a QR — `pay_invoice_popup_layout` forces the text card so the user copies the invoice or cancels the order instead of paying a dead invoice.
