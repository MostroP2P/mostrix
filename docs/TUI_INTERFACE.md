...
- **Invoice popups (`NewMessageNotification`)**:
  ...
  - `PayInvoice` and `PayBondInvoice` show a half-block QR when it fits (`src/ui/helpers/qr.rs`). **SPACE** toggles QR/text, **C** copies the raw bolt11, and **Up/Down** / **PageUp/PageDown** scroll the text view. If the terminal cannot fit the QR, the popup falls back to the wrapped invoice immediately. Cancel selection is still available.
