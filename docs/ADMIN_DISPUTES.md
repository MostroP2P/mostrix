...
- **Receiving messages**:
  ...
  - Hydrate `since` uses `last_seen_timestamp`; already-accepted inner event ids are skipped from the durable `.inner_ids` set (the router does not consume an id on emit).
