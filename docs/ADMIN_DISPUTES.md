...
- **Cipher**: Blob layout is nonce (12 bytes) + ciphertext + authentication tag (16 bytes); decryption uses the `chacha20poly1305` crate. Max blob size is 25 MB per download. **Blossom HTTP**: save uses unauthenticated GET by URL (`fetch_blob`); My Trades **outbound upload** (Phase B) uses NIP-24242 PUT auth signed with the order **trade key** — see [MESSAGE_FLOW_AND_PROTOCOL.md](MESSAGE_FLOW_AND_PROTOCOL.md) — "Attachments (send — Phase B backend)".
