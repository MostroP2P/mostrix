...
- **Behavior**: When empty, My Trades attachment **upload** uses `DEFAULT_BLOSSOM_SERVERS` in `src/util/blossom.rs`. When non-empty, `upload_blob_with_retry` tries each HTTPS base in order until one accepts the PUT.
