...
- **`blossom_servers`**: HTTPS Blossom bases for **My Trades attachment upload** (**Ctrl+O**). When empty, Mostrix uses `DEFAULT_BLOSSOM_SERVERS` in `src/util/blossom.rs` (same defaults as Mostro Mobile). **Settings tab** (User and Admin): **Add Blossom Server**, **Remove Blossom Server**, **Restore Default Blossom Servers** — same add → confirm / pick → confirm / restore-confirm flow as relays. A bare host gets `https://`. If the on-disk list is empty, the first add or remove materializes the built-in defaults so a single add does not replace them. The last server cannot be removed. Restore writes the compiled default list. Example in repo `settings.toml`: commented `# blossom_servers = ["https://cdn.hzrd149.com", …]`. Resolved at send time via `blossom_servers_from_settings` (main loop reloads settings from disk when draining the send queue). A failed host is skipped; send fails only after every configured server has been tried.
...
- **`AppState.transport`**: resolved wire transport for **protocol DMs** via [`transport_from_instance`](../src/util/mostro_info.rs). Updated through [`AppState.set_mostro_info`](../src/ui/app_state.rs) (startup await, main loop `MostroInfoFetchResult`, reconnect, invalid-pubkey clear; ignores older `created_at` than the cache **for the same Mostro pubkey** — switching coordinators always replaces). Kind 38385 cache carries the event author; confirming a new Mostro pubkey clears the cache immediately; late fetch replies for a non-active pubkey are ignored.
- **Startup**: when relays are reachable, [`run_post_terminal_startup`](../src/startup.rs) **awaits** [`fetch_mostro_instance_info`](../src/util/mostro_info.rs) before spawning the DM listener so PoW and instance metadata are available. Protocol DM subscriptions are always kind 14. On fetch failure or offline boot, transport defaults to NIP-44.
- **Auth (MOSTRO-075)**: relay `.author()` filters are not trusted. [`fetch_mostro_instance_info`](../src/util/mostro_info.rs) re-checks `event.pubkey`, the `d` tag, and `Event::verify` via [`instance_info_event_is_authentic`](../src/util/mostro_info.rs) / [`select_authentic_instance_info_event`](../src/util/mostro_info.rs) before apply. [`MostroInstanceInfoFetch::Rejected`](../src/util/mostro_info.rs) / `NotFound` preserve the cached revision when it still matches the active Mostro; after a coordinator switch, mismatched cache is cleared. Relay fetch failures use [`MostroInfoFetchResult::FetchFailed`](../src/ui/orders.rs) (preserve). Explicit clears stay on hard [`MostroInfoFetchResult::Err`](../src/ui/orders.rs) (invalid settings/pubkey).

Displayed on the **Mostro Info** tab: protocol version (`1` / `2` / unknown) and wire transport label (NIP-44 direct). Protocol v1 advertisements show an unsupported warning.
...
5. **Shared-key chat subscription router** (`listen_for_chat_messages` in `src/util/chat_listener.rs`):
   ...
   - On `TrackChatKey`, the router hydrates history once via kind-14 fetch (`fetch_chat_messages_for_shared_key`: `authors = [pub(K_sign)]`; filtered by the last-seen cursor; unwrapped with the track's inner-signer allow-list). Buffered track/untrack commands are drained and applied before a **single** batched resubscribe (idempotent re-tracks are no-ops). Subscribe failures retry with short backoff. Empty `allowed_signers` lists are not tracked.
   - Live traffic is kind 14, routed by the derived `pub(K_sign)` outer author. Events are decrypted with `unwrap_chat_envelope` against the track’s inner-signer allow-list and emitted on the existing `admin_chat_updates` / `user_order_chat_updates` channels. Outbound chat is always kind 14. Inner-event replay protection uses the durable `.inner_ids` set recorded after transcript save; the router does not consume an inner id on emit. The session outer-id LRU retains an event id only after that durable skip or a poison unwrap, so the same outer event can be retried after a failed transcript write.
   - **Application**: `ui::helpers::apply_admin_chat_updates` / `apply_user_order_chat_updates` (in `src/ui/helpers/startup.rs`) merge updates, persist transcripts, record inner ids only after a successful save, and advance last-seen cursors (`buyer_chat_last_seen` / `seller_chat_last_seen`, `order_chat_last_seen`) only when every write in the batch succeeded. Unknown dispute-chat inner signers are dropped (not labeled Admin).
