...
5. **Shared-key chat subscription router** (`listen_for_chat_messages` in `src/util/chat_listener.rs`):
   ...
   - Live traffic is kind 14, routed by the derived `pub(K_sign)` outer author. Events are decrypted with `unwrap_chat_envelope` against the track’s inner-signer allow-list and emitted on the existing `admin_chat_updates` / `user_order_chat_updates` channels. Outbound chat is always kind 14.
