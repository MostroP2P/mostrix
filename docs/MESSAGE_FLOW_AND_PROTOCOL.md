...
- **Saved buyer Lightning address (`settings.toml` → `ln_address`)**: If the trimmed address is non-empty when **`AddInvoice`** should open (incoming notification handled by **`handle_message_notification`**, or **Messages → Enter** via **`present_add_invoice_popup`** in `src/util/dm_utils/notifications_ch_mng.rs`), the client shows **`UiMode::ConfirmSavedLnAddressForInvoice`** (YES/NO). **YES** inserts **`BuyerInvoicePreference::UseSavedLnAddress`** for that **`order_id`** (when applicable) and immediately runs **`submit_add_invoice`** (`src/ui/key_handler/message_handlers.rs`) with the saved address — **`execute_add_invoice`** in the background, UI **`WaitingAddInvoice`** — without opening **`NewMessageNotification`** for a second submit step; that path is handled only from **`handle_enter_key`** (`src/ui/key_handler/enter_handlers.rs`), not duplicated in **`handle_confirm_key`**. **NO** inserts **`ManualInvoice`** via **`apply_saved_ln_address_invoice_choice`** and opens **`NewMessageNotification`** with an empty invoice field. The confirmation popup body lists the address string read from disk during **`ui_draw`**.
