...
- **Attachments (send — Phase B backend)**: Outbound sends are implemented in `src/util/send_attachment.rs` but **not yet exposed in the TUI** (Phase C will add **Ctrl+O** / file picker). The pipeline is:
  ...
  6. **UI feedback** — success: `OperationResult::OrderChatAttachmentSent` → append **You** row, JSON transcript save, `Info` popup. **Upload ok / send failed**: `OrderChatAttachmentSendFailed` stores `PreparedOrderChatAttachment` in `AppState.pending_order_attachment_sends` and shows an `Error` popup with the Blossom URL (retry via `SendOrderAttachmentJob::RetryPrepared` without re-upload — Phase C UI).
