...
- **Save completion UI**: `main.rs` drains `save_attachment_rx` and `order_result_rx` before every `terminal.draw` so Blossom download results surface without an extra keypress (see [STARTUP_AND_CONFIG.md](STARTUP_AND_CONFIG.md) — Main Event Loop).
