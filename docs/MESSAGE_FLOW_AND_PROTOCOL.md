...
- **Application**: The main loop receives results on `admin_chat_updates_rx` and applies them via `apply_admin_chat_updates`, which:
  ...
  - Records inner event ids only after durable transcript save. The live/hydrate router consults that `.inner_ids` set and does not consume an inner id on emit.
