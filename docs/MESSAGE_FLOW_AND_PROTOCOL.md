...
| **P2P order chat** (My Trades) and **admin dispute chat** | Kind 14 (`K_sign` / `K_conv`) via `mostro_core::chat` | Unrelated to protocol v2 Mostro DMs; ECDH secret still stored, keys derived at runtime |
...
2. **NIP-59 Gift Wrap (1059)**: **not** used for protocol DMs or P2P / dispute chat.
...
- **Live relay sync (User role)**: the **shared-key chat subscription router** (`listen_for_chat_messages` in `src/util/chat_listener.rs`) maintains a batched kind-14 subscription (`authors = [pub(K_sign)]`) over all active order chats and routes by outer author. `track_startup_chats` seeds the active-order set at startup; the DM router tracks/untracks orders when the shared key becomes resolvable or the order hits a chat-terminal status ([`TERMINAL_DM_STATUSES`](../src/models.rs) — **`success` keeps chat live**). Dynamic tracks pass a hydrate `since` from the on-disk transcript max timestamp when present. Shared keys come from persisted `order_chat_shared_key_hex` when set, otherwise ECDH from local `trade_keys` + `counterparty_pubkey` (`src/util/chat_utils.rs`).
- **Waking the counterparty (push notification)**: after a relay accepts a **peer** order chat send (text or attachment), Mostrix wakes `counterparty_pubkey` via `mostro-push-server` `POST {push_server_url}/api/notify` — same fire-and-forget, delivery-gated (`wake_target`), 10 s per-recipient debounce as dispute chat. The **Solver** channel is never woken (the solver is not a push client). Disabled when `push_server_url` is empty. See `src/util/push.rs` and [ADMIN_DISPUTES.md](ADMIN_DISPUTES.md).
- **Incremental merge**: `apply_user_order_chat_updates` in `src/ui/helpers/startup.rs`:
  ...
  - Persists new entries with `save_order_chat_message` / `rewrite_order_chat_messages` and advances per-order `order_chat_last_seen`. Inner event ids are recorded only after durable transcript save succeeds. The live/hydrate router consults that durable `.inner_ids` set and does **not** consume an inner id on emit. The live outer-id LRU likewise retains an event id only after that durable skip (or a poison unwrap), so a relay redelivery of the same Nostr event can retry a failed save without restarting the listener. A persist failure also leaves the last-seen cursor unadvanced so hydrate/`since` cannot skip the gap.
...
**Source**: `src/ui/helpers/startup.rs`, `src/ui/helpers/chat_storage.rs`, `src/ui/helpers/chat_visibility.rs`, `src/ui/helpers/attachments.rs`, `src/ui/helpers/order_chat_projection.rs`, `src/ui/save_attachment_popup.rs`, `src/ui/send_attachment_picker.rs`, `src/util/dm_utils/order_ch_mng.rs`, `src/util/chat_utils.rs`, `src/util/blossom.rs`, `src/util/file_validation.rs`, `src/util/send_attachment.rs`, `src/util/push.rs`
...
When the user is in **Admin** mode, the shared-key chat subscription router keeps the "Disputes in Progress" tab up to date with kind-14 chat events exchanged over **per‑dispute shared keys** — live via a relay subscription, not a timer.
...
- **Router**: `listen_for_chat_messages` (`src/util/chat_listener.rs`) subscribes kind 14 by `authors = [pub(K_sign)]`. Hydration (`fetch_chat_messages_for_shared_key`, 7‑day window, `last_seen` cursor, party+admin allow-list) uses the same kind-14 filter. Live kind 14 is routed by `pub(K_sign)`. Outbound chat is **only** kind 14.
- **Application**: The main loop receives results on `admin_chat_updates_rx` and applies them via `apply_admin_chat_updates`, which:
  ...
  - Records inner event ids only after durable transcript save. The live/hydrate router consults that `.inner_ids` set and does not consume an inner id on emit. Live outer-event ids are retained in the session LRU only after that durable skip or a poison unwrap.
  - Updates in‑memory `admin_chat_last_seen` and persists cursors to the `admin_disputes` table (`buyer_chat_last_seen`, `seller_chat_last_seen`) via `update_chat_last_seen_by_dispute_id` only when every transcript write in the batch succeeded.
